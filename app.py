...
# Reflection Tracker shortcut
@app.route("/reflection")
def reflection_tracker_shortcut():
    from flask import redirect
    return redirect("/first-nine-weeks/day/45?view=student")
# End Reflection Tracker shortcut
...
@app.route("/labs/science-assessment")
def science_studio_assessment_builder():
    from flask import render_template
    from curriculum.staar_2022_practice import get_all_staar_2022_questions

    def normalize_teks(teks):
        return str(teks or "Unlisted").replace("(", "").replace(")", "").upper().strip()

    def normalize_options(options):
        if isinstance(options, dict):
            return [
                {"letter": str(letter).upper(), "text": str(text)}
        ...
        if isinstance(options, list):
            ...
            for index, option in enumerate(options):
                if isinstance(option, dict):
                    letter = str(option.get("letter") or option.get("key") or letters[index]).upper()
    ...
    for question in get_all_staar_2022_questions():
        ...
        clean_questions.append({
            ...
            "source_item": question.get("source_item", ""),
            "year": question.get("year", "2022/2025"),
            "day": question.get("day", ""),
            "topic": question.get("topic", "Science Review"),
            "source_teks": normalize_teks(question.get("source_teks", "")),
            "process_teks": normalize_teks(question.get("process_teks", "")),
            ...
            "image": get_visual_image(question),
            "stimulus": question.get("stimulus", ""),
        })

    return render_template(
        "science_assessment_builder.html",
