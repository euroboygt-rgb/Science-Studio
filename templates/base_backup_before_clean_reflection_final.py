...
<!-- Science Studio Reflection Score Final Display Fix -->
<script>
(function () {
    function isReflectionPage() {
        const path = window.location.pathname.toLowerCase();
    ...
    function fieldContext(field) {
        return [
            ...
            field.getAttribute("aria-label") || "",
            field.closest("label") ? field.closest("label").innerText : "",
            field.closest("div") ? field.closest("div").innerText.slice(0, 300) : "",
            field.closest("section") ? field.closest("section").innerText.slice(0, 450) : ""
        ].join(" ").toLowerCase();
    }
    ...
    function findAssessmentScoreCard() {
        const all = Array.from(document.querySelectorAll("section, article, div"));

        const cards = all.filter(function (element) {
            const text = element.innerText || "";
            return text.includes("Assessment Score");
        });
    ...
    function hideOldDashScore(card) {
        if (!card) return;
        ...
        all.forEach(function (element) {
            ...
            if (text === "--%" || /^\d{1,3}%$/.test(text)) {
                if (!element.id || element.id !== "scienceStudioLiveAssessmentScore") {
                    element.classList.add("science-studio-old-score-hidden");
                }
            }
        });
    }

    function updateBigAssessmentScore(score) {
        const card = findAssessmentScoreCard();

        if (!card) return;

        hideOldDashScore(card);

        let display = document.getElementById("scienceStudioLiveAssessmentScore");

        if (!display) {
            display = document.createElement("div");
            display.id = "scienceStudioLiveAssessmentScore";
            display.className = "science-studio-live-assessment-score";

            card.insertBefore(display, card.firstChild);
        }
    ...
    function scoreMessage(score) {
        ...
        if (score >= 80) {
            return "Strong work. Use your score to set your next learning goal.";
        }

        if (score >= 70) {
            return "Good progress. Pick one topic to review and improve.";
        }

        if (score >= 60) {
            return "You are building skill. Choose one TEKS to practice again.";
        }

        return "This score is useful evidence. Pick one skill to practice and grow.";
    }

    function updateScoreReflection(score) {
        const card = findScoreReflectionCard();

        if (!card) return;

        let panel = document.getElementById("scienceStudioLiveScoreReflection");

        if (!panel) {
            panel = document.createElement("div");
            panel.id = "scienceStudioLiveScoreReflection";
            panel.className = "science-studio-live-score-reflection";
            panel.innerHTML = `
                <div class="science-studio-live-score-track">
                    <div class="science-studio-live-score-fill" id="scienceStudioLiveScoreFill"></div>
                </div>
                <div class="science-studio-live-score-message" id="scienceStudioLiveScoreMessage"></div>
            `;

            card.appendChild(panel);
        }

        const fill = document.getElementById("scienceStudioLiveScoreFill");
        const message = document.getElementById("scienceStudioLiveScoreMessage");
        ...
        if (message) {
            message.textContent = scoreMessage(score);
        }
    }
    ...
    function refreshScoreDisplay() {
        ...
        updateBigAssessmentScore(score);
        updateScoreReflection(score);
    }

    document.addEventListener("input", function () {
        setTimeout(refreshScoreDisplay, 50);
    }, true);
    ...
    document.addEventListener("click", function () {
        setTimeout(refreshScoreDisplay, 100);
        setTimeout(refreshScoreDisplay, 400);
    }, true);

    window.addEventListener("load", function () {
        setTimeout(refreshScoreDisplay, 100);
        setTimeout(refreshScoreDisplay, 500);
        setTimeout(refreshScoreDisplay, 1000);
    });

    setTimeout(refreshScoreDisplay, 250);
    setTimeout(refreshScoreDisplay, 750);
})();
</script>
<!-- End Science Studio Reflection Score Final Display Fix -->


<!-- Science Studio Reflection Bar Final Fix -->
<script>
(function () {
    function isReflectionPage() {
        const path = window.location.pathname.toLowerCase();
    ...
    function getScore() {
        ...
        return parseScore(scoreField.value);
    }

    function smallestBoxContaining(textToFind) {
        const boxes = Array.from(document.querySelectorAll("section, article, div"));

        const matches = boxes.filter(function (box) {
    ...
    function rebuildAssessmentScoreBox(score) {
        const box = smallestBoxContaining("Assessment Score");

        if (!box) {
            return;
        }

        if (!box.classList.contains("science-studio-score-box-fixed")) {
            box.classList.add("science-studio-score-box-fixed");
        }

        box.innerHTML = `
            <div class="science-studio-score-number">${score === null ? "--%" : score + "%"}</div>
            <div class="science-studio-score-label">Assessment Score</div>
        `;
    }

    function rebuildScoreReflectionBox(score) {
        const box = smallestBoxContaining("Score Reflection");

        if (!box) {
            return;
        }

        if (!box.classList.contains("science-studio-score-reflection-fixed")) {
            box.classList.add("science-studio-score-reflection-fixed");
        }
        ...
        box.innerHTML = `
            ...
            <div class="science-studio-reflection-score-track">
                <div class="science-studio-reflection-score-fill" style="width: ${width}%"></div>
            </div>
            <div class="science-studio-reflection-score-message">
                ${scoreMessage(score)}
            </div>
        `;
    }

    function refreshReflectionScore() {
        if (!isReflectionPage()) {
            return;
        }
        ...
        rebuildAssessmentScoreBox(score);
        rebuildScoreReflectionBox(score);
    }

    document.addEventListener("input", function () {
        setTimeout(refreshReflectionScore, 50);
    }, true);
    ...
    document.addEventListener("click", function () {
        setTimeout(refreshReflectionScore, 100);
        setTimeout(refreshReflectionScore, 500);
    }, true);

    window.addEventListener("load", function () {
        setTimeout(refreshReflectionScore, 100);
        setTimeout(refreshReflectionScore, 500);
        setTimeout(refreshReflectionScore, 1000);
    });

    setTimeout(refreshReflectionScore, 250);
    setTimeout(refreshReflectionScore, 750);
})();
</script>
<!-- End Science Studio Reflection Bar Final Fix -->
